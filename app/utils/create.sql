...
CREATE TABLE ITR_ARPT(CD_ARPT varchar(3) UNIQUE NOT NULL PRIMARY KEY,
						...
						FOREIGN KEY(CD_PAIS) REFERENCES ITR_PAIS(CD_PAIS) ON DELETE CASCADE,
						FOREIGN KEY(SG_UF) REFERENCES ITR_UF(SG_UF) ON DELETE SET NULL
...
CREATE TABLE ITR_ARNV(CD_ARNV varchar(5) UNIQUE NOT NULL PRIMARY KEY,
						...
						FOREIGN KEY(CD_EQPT) REFERENCES ITR_EQPT(CD_EQPT) ON DELETE CASCADE,
						FOREIGN KEY(CD_CMPN_AEREA) REFERENCES ITR_CMPN_AEREA(CD_CMPN_AEREA) ON DELETE CASCADE
);



CREATE TABLE ITR_ROTA_VOO(NR_ROTA_VOO int UNIQUE NOT NULL PRIMARY KEY AUTO_INCREMENT,
							CD_ARPT_ORIG varchar(3) NOT NULL,
							CD_ARPT_DEST varchar(3) NOT NULL,
							VR_PASG int,
							FOREIGN KEY(CD_ARPT_ORIG) REFERENCES ITR_ARPT(CD_ARPT) ON DELETE CASCADE,
							FOREIGN KEY(CD_ARPT_DEST) REFERENCES ITR_ARPT(CD_ARPT) ON DELETE CASCADE
...
CREATE TABLE ITR_PSGR (CD_PSGR int UNIQUE NOT NULL PRIMARY KEY AUTO_INCREMENT,
						...
						CD_PSGR_RESP int,
						FOREIGN KEY (CD_PAIS) REFERENCES ITR_PAIS(cd_pais) ON DELETE SET NULL,
						FOREIGN KEY (CD_PSGR_RESP) REFERENCES ITR_PSGR(CD_PSGR) ON DELETE SET NULL,
...
CREATE TABLE ITR_VOO (NR_VOO int NOT NULL,
						...
						FOREIGN KEY(CD_ARNV) REFERENCES ITR_ARNV(CD_ARNV) ON DELETE CASCADE,
						FOREIGN KEY(NR_ROTA_VOO) REFERENCES ITR_ROTA_VOO(NR_ROTA_VOO) ON DELETE CASCADE
...
CREATE TABLE ITR_RESV(CD_PSGR int NOT NULL,
						...
						DT_SAIDA_VOO varchar(10) NOT NULL,
						PC_DESC_PASG int,
						FOREIGN KEY (CD_PSGR) REFERENCES ITR_PSGR(CD_PSGR) ON DELETE CASCADE,
                      	FOREIGN KEY (NR_VOO,DT_SAIDA_VOO) REFERENCES ITR_VOO(NR_VOO,DT_SAIDA_VOO) ON DELETE CASCADE
                      );
